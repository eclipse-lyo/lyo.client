...
public class OAuthRedirectException extends OAuthException {
    ...
    /**
     * @param callbackURL the URL to redirect to, after the user approves the request token. (can be null)
     * @return the complete redirect URL, including the request token, and callback URL
     * @throws UnsupportedEncodingException 
     */
    public String getRedirectQuery(String callbackURL) throws UnsupportedEncodingException {
        return redirectURL + 
                "?oauth_token=" + accessor.requestToken +
                (StringUtils.isEmpty(callbackURL) ? "" : "&oauth_callback="+URLEncoder.encode(callbackURL, "UTF-8"));
    }
