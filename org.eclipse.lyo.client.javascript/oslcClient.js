...
(function(exports, $, undefined) {
  ...
  OslcClient.prototype._dialogCommonHandle = function(callback, errorCallback, draftResource) {
    ...
    return function(dialogResource) {
      ...
      function prepareDialog(dialogUrl) {
        ...
        var dialog = jQuery("<div id='dialog'></div>").append(iframe).appendTo("body").dialog({
          autoOpen: false,
          modal: true,
          resizable: false,
          width: width,
  ...
  OslcClient.prototype.lookupServiceProviderUrl = function(catalogUrl, serviceProviderTitle, callback, errorCallback) {
    ...
    function internalCallBack(data, textStatus, jqXHR) {
      ...
      var nodes=jQuery(data).find("oslc\\:ServiceProviderCatalog > oslc\\:serviceProvider > oslc\\:ServiceProvider > dcterms\\:title, ServiceProviderCatalog > serviceProvider > ServiceProvider > title");

      //if nodes is empty, find without typed nodes
      if(nodes.length===0){
        nodes=jQuery(data).find("rdf\\:Description > rdf\\:type[rdf\\:resource='http://open-services.net/ns/core#ServiceProvider'], Description > type[resource='http\\://open-services.net/ns/core#ServiceProvider']").parent().find( "dcterms\\:title,title");
  ...
  OslcClient.prototype.lookupCreationDialogResource = function(serviceProviderUrl, oslcDomain, oslcResourceType, callback, errorCallback) {
    ...
    function internalCallBack(data, textStatus, jqXHR) {
      ...
      var nodes=jQuery(data).find("oslc\\:ServiceProvider > oslc\\:service > oslc\\:Service, ServiceProvider > service > Service");


      //if nodes is empty, find without typed nodes
    if(nodes.length===0){
     var serviceRDFNodeId=jQuery(data).find("rdf\\:Description > rdf\\:type[rdf\\:resource='http://open-services.net/ns/core#ServiceProvider'], Description > type[resource='http\\://open-services.net/ns/core#ServiceProvider']  ").parent().find("oslc\\:service , service").attr("rdf:nodeID");
    nodes=jQuery(data).find("rdf\\:Description[rdf\\:nodeID='"+serviceRDFNodeId+"'] , Description[nodeID='"+serviceRDFNodeId+"']");
    }
      ...
      nodes.each(function() {
        ...
        jQuery(this).find("oslc\\:domain,domain").each(function() {
          ...
          if (currentDomain !== null && currentDomain == oslcDomain) {
            ...
            var dialogNode=jQuery(this).parent().find("oslc\\:creationDialog > oslc\\:Dialog , creationDialog > Dialog");

            //if dialogNode is empty, find without typed nodes
            if(dialogNode.length===0){
             var  dialogElements=[];
              jQuery(this).parent().find("oslc\\:creationDialog , creationDialog").each(function() {
                dialogNodeRdfID= jQuery(this).attr("rdf:nodeID");
                var currDialogNode=jQuery(this).parent().parent().find("rdf\\:Description[rdf\\:nodeID='"+dialogNodeRdfID+"'] , Description[nodeID='"+dialogNodeRdfID+"']");
                dialogElements.push(currDialogNode);
              });
              dialogNode=jQuery(dialogElements);
            }

            dialogNode.each(function() {
      ...
      nodes.each(function() {
        ...
        jQuery(this).find("oslc\\:domain,domain").each(function() {
          ...
          if (currentDomain !== null && currentDomain == oslcDomain) {
            ...
            var dialogNode=jQuery(this).parent().find("oslc\\:selectionDialog > oslc\\:Dialog , selectionDialog > Dialog");

            //if dialogNode is empty, find without typed nodes
            if(dialogNode.length===0){
             var  dialogElements=[];
              jQuery(this).parent().find("oslc\\:selectionDialog , selectionDialog").each(function() {
                dialogNodeRdfID= jQuery(this).attr("rdf:nodeID");
                var currDialogNode=jQuery(this).parent().parent().find("rdf\\:Description[rdf\\:nodeID='"+dialogNodeRdfID+"'] , Description[nodeID='"+dialogNodeRdfID+"']");
                dialogElements.push(currDialogNode);
              });
      ...
      nodes.each(function() {
        jQuery(this).find("oslc\\:domain,domain").each(function() {
          ...
          if (currentDomain !== null && currentDomain == oslcDomain) {
            ...
            var queryCapabilityNodes=jQuery(this).parent().find("oslc\\:queryCapability > oslc\\:QueryCapability , queryCapability > QueryCapability");

            //if queryCapabilityNodes is empty, find without typed nodes
            if(queryCapabilityNodes.length===0){
             var  queryCapabilityElements=[];
              jQuery(this).parent().find("oslc\\:queryCapability , queryCapability").each(function() {
                queryCapabilityNodeRdfID= jQuery(this).attr("rdf:nodeID");
                var currQueryCapabilityNode=jQuery(this).parent().parent().find("rdf\\:Description[rdf\\:nodeID='"+queryCapabilityNodeRdfID+"'] , Description[nodeID='"+queryCapabilityNodeRdfID+"']");
                queryCapabilityElements.push(currQueryCapabilityNode);
              });
              queryCapabilityNodes=jQuery(queryCapabilityElements);
            }

            queryCapabilityNodes.each(function() {
      ...
      nodes.each(function() {
        ...
        jQuery(this).find("oslc\\:domain,domain").each(function() {
          ...
          if (currentDomain !== null && currentDomain == oslcDomain) {
            ...
            var creationFactoryNodes=jQuery(this).parent().find("oslc\\:creationFactory > oslc\\:CreationFactory , creationFactory > CreationFactory");

            //if creationFactoryNodes is empty, find without typed nodes
            if(creationFactoryNodes.length===0){
             var  creationFactoryElements=[];
              jQuery(this).parent().find("oslc\\:creationFactory , creationFactory").each(function() {
                creationFactoryNodeRdfID= jQuery(this).attr("rdf:nodeID");
                var currCreationFactoryNode=jQuery(this).parent().parent().find("rdf\\:Description[rdf\\:nodeID='"+creationFactoryNodeRdfID+"'] , Description[nodeID='"+creationFactoryNodeRdfID+"']");
                creationFactoryElements.push(currCreationFactoryNode);
              });
              creationFactoryNodes=jQuery(creationFactoryElements);
            }

            creationFactoryNodes.each(function() {
